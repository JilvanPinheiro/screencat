...
module.exports.remote = function (peerConnection, ui, config, room) {
  peerConnection.remotePeer(config, room, function (err, peer) {
    if (err) {
      ui.inputs.paste.value = 'Error on remotePeer! ' + err.message
      return
    }

    if (!room) {
      ui.inputs.paste.value = 'Error on room value! Please Quit'
      return
    }
    ...
    peer.on('signal', function (sdp) {
      ...
      peerConnection.handleSignal(sdp, peer, true, room, function (err) {
        if (err) {
          ui.containers.content.innerHTML = 'Error on handleSignal! Please Quit. ' + err.message
          return
        }
...
module.exports.host = function (peerConnection, ui, opts) {
  ...
  getARoom(peerConnection, ui, function (err, room, config) {
    if (err) {
      ui.inputs.copy.value = 'Error on getARoom! ' + err.message
      return
    }
    ...
    peerConnection.hostPeer(opts, function (err, peer) {
      if (err) {
        ui.inputs.copy.value = 'Error on hostPeer! ' + err.message
        return
      }

      if (!room) {
        ui.inputs.copy.value = 'Error on room! Please Quit'
        return
      }
      ...
      peer.on('signal', function (sdp) {
        peerConnection.handleSignal(sdp, peer, false, room, function (err) {
          if (err) {
            ui.containers.content.innerHTML = 'Error on signal! Please Quit. ' + err.message
            return
          }
        })
      })
