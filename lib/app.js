...
ui.buttons.share.addEventListener('click', function (e) {
  ...
  try {
    if (!peerConnection.robot) peerConnection.robot = require('../lib/robot.js')
  } catch (e) {
    error(new Error('../lib/robot.js failed to load'))
    error(e)
  }
...
ipc.on('join-room', (ev, t) => {
  joinRoom(t)
}) 

// to expose to ipc.joinRoom
const joinRoom = (roomId) => {
  ...
  peerConnection.getRemoteConfig((err, config) => {
    if (err) {
      console.log(err)
      ui.inputs.paste.value = 'Error on getRemoteConfig! ' + err.message
      return
    }
...
ui.buttons.join.addEventListener('click', function (e) {
  ...
  connect.verifyUserRoom(peerConnection, ui, function (err, room, config) {
    ...
    if (err) {
      ui.inputs.paste.value = 'Error on verifyUserRoom! ' + err.message
      return
    }
