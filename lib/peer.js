...
var roomName = "poop-poop-snack-snack"

module.exports = function create () {
  //;var server = 'http://catlobby.maxogden.com'
  var server = 'http://ec2-3-94-186-140.compute-1.amazonaws.com:5005'
  var remoteConfigUrl = ''
  ...
  function getRemoteConfig (cb) {
	config = {iceServers: [{urls: 'stun:35.196.118.78:3478'},{urls: 'turn:35.196.118.78:3478'}]}
    cb(null, config)
  ...
  function createRoom (cb) {
    nets({method: 'POST', uri: server + '/v1?name=' + roomName}, function response (err, resp, body) {
      if (err) return cb(err)
