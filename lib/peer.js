...
var getUserMedia = require('../lib/get-user-media.js')()

module.exports = function create () {
  //;var server = 'http://catlobby.maxogden.com'
  var server = 'https://konech-app-backend.herokuapp.com/'
  var remoteConfigUrl = ''
  ...
  function verifyRoom (room, cb) {
    ...
    nets({method: 'POST', uri: server + '/rc-api/' + room + '/pong', json: {ready: true}}, function response (err, resp, data) {
  ...
  function remotePeer (config, room, cb) {
    // listen for pings
    var pingsUrl = server + '/rc-api/' + room + '/pings'
    ...
    events.onmessage = function onMessage (e) {
      ...
      inflate(row.data, function inflated (err, stringified) {
        ...
        getAudio(function got (err, audioStream) {
          ...
          var peer = new SimplePeer({ config: config })
  ...
  function createRoom (cb) {
    nets({method: 'POST', uri: server + '/rc-api/init'}, function response (err, resp, body) {
      if (err) return cb(err)
  ...
  function hostPeer (opts, cb) {
    ...
    // listen for pongs
    var events = new EventSource(server + '/rc-api/' + room + '/pongs')
    events.onmessage = function onMessage (e) {
      ...
      function connect (pong) {
        ...
        getUserMedia(constraints, function (videoStream) {
          ...
          getUserMedia({audio: true, video: false}, function (audioStream) {
            peer = new SimplePeer({ initiator: true, config: config })
  ...
  function handleSignal (sdp, peer, remote, room, cb) {
    deflate(sdp, function deflated (err, data) {
      ...
      // upload sdp
      var uploadURL = server + '/rc-api/' + room
